...
var PortWrapper = /** @class */ (function (_super) {
    ...
    function PortWrapper() {
        ...
        _this.onMouseDown = function (startEvent) {
            var _a = _this.props, offset = _a.offset, node = _a.node, port = _a.port, onLinkStart = _a.onLinkStart, onLinkCancel = _a.onLinkCancel, onLinkComplete = _a.onLinkComplete, onLinkMove = _a.onLinkMove, config = _a.config, hovered = _a.hovered;
            var linkId = uuid_1.v4();
            var fromNodeId = node.id;
            var fromPortId = port.id;
            if (config.getPortInfo) {
                var portInfo = config.getPortInfo({ fromNodeId: fromNodeId, fromPortId: fromPortId, hovered: hovered });
                ...
                if (portInfo.linkId)
                    linkId = portInfo.linkId;
                if (portInfo.fromNodeId)
                    fromNodeId = portInfo.fromNodeId;
                if (portInfo.fromPortId)
                    fromPortId = portInfo.fromPortId;
            }
        ...
        _this.onMouseOver = function (startEvent) {
            var _a = _this.props, config = _a.config, node = _a.node, port = _a.port, onPortEnter = _a.onPortEnter;
            // Notify state of link start
            onPortEnter({ config: config, node: node, port: port, isHovering: true });
            // Prevent default and stop propagation to prevent text selection
            startEvent.preventDefault();
            startEvent.stopPropagation();
        };
        _this.onMouseOut = function (startEvent) {
            var _a = _this.props, config = _a.config, node = _a.node, port = _a.port, onPortEnter = _a.onPortEnter;
            // Notify state of link start
            onPortEnter({ config: config, node: node, port: port, isHovering: false });
            // Prevent default and stop propagation to prevent text selection
            startEvent.preventDefault();
            startEvent.stopPropagation();
        };
        _this.onMouseDownCatch = function () {
    ...
    PortWrapper.prototype.render = function () {
        ...
        return (React.createElement("div", { "data-port-id": port.id, "data-node-id": node.id, onMouseDown: (!config.propagatePortClick) ? this.onMouseDown : this.onMouseDownCatch, onMouseOver: this.onMouseOver, onMouseOut: this.onMouseOut, ref: this.nodeRef, style: style },
            React.createElement(Component, { config: config, node: node, port: port, context: this.context, onMouseDown: (config.propagatePortClick) ? this.onMouseDown : this.onMouseDownCatch, isNodeSelected: !!selected && selected.type === 'node' && selected.id === node.id, isSelected: !!selected && selected.type === 'port' && selected.id === port.id, isHovered: !!hovered && hovered.type === 'port' && hovered.id === port.id && hovered.nodeId === node.id, isLinkSelected: selectedLink
