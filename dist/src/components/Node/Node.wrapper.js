...
exports.NodeWrapper = function (_a) {
    var config = _a.config, node = _a.node, onDragNode = _a.onDragNode, onNodeClick = _a.onNodeClick, onDragStop = _a.onDragStop, isSelected = _a.isSelected, _b = _a.Component, Component = _b === void 0 ? Node_default_1.NodeDefault : _b, onNodeSizeChange = _a.onNodeSizeChange, NodeInner = _a.NodeInner, Ports = _a.Ports, Port = _a.Port, offset = _a.offset, selected = _a.selected, selectedLink = _a.selectedLink, hovered = _a.hovered, hoveredLink = _a.hoveredLink, onPortPositionChange = _a.onPortPositionChange, onLinkStart = _a.onLinkStart, onLinkMove = _a.onLinkMove, onLinkComplete = _a.onLinkComplete, onLinkCancel = _a.onLinkCancel, onPortEnter = _a.onPortEnter;
    ...
    var children = (React.createElement(React.Fragment, null,
        ...
        React.createElement(Ports, { config: config }, Object.keys(node.ports).map(function (portId) {
            var style = {};
            return (React.createElement(__1.PortWrapper, { config: config, style: style, key: portId, offset: offset, selected: selected, selectedLink: selectedLink, hoveredLink: hoveredLink, hovered: hovered, node: node, port: node.ports[portId], Component: Port, onPortPositionChange: onPortPositionChange, onLinkStart: config.readonly ? utils_1.noop : onLinkStart, onLinkMove: config.readonly ? utils_1.noop : onLinkMove, onLinkComplete: onLinkComplete, onLinkCancel: onLinkCancel, onPortEnter: onPortEnter }));
        }))));
    var zoomScale = React.useContext(CanvasContext_1.default).zoomScale;
    return (React.createElement(react_draggable_1.default, { bounds: "parent", axis: "both", position: node.position, grid: [1, 1], scale: zoomScale, onStart: function (e) {
            // Stop propagation so the canvas does not move
            e.stopPropagation();
        }, onDrag: function (event, data) { return onDragNode({ config: config, event: event, data: data, id: node.id }); }, onStop: function (event, data) { return onDragStop({ config: config, event: event, data: data, id: node.id }); }, disabled: config.readonly },
        React.createElement(Component, { config: config, ref: compRef, children: children, onClick: function (e) {
            ...
            }, isSelected: isSelected, node: node, hovered: hovered })));
};
